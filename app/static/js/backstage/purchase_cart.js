...
export function init() {
    console.log('purchase_cart.js 模組初始化');
    // 綁定在容器上（比如整個子頁區塊）
    const container = document.getElementById("purchase-cart-page");
    
    if (!container) {
        console.error('找不到 purchase-cart-page 元素');
        return;
    }
    ...
    console.log('purchase_cart.js 事件監聽器已設置');
}
...
async function getCartContent() {
    try {
        const response = await fetch(`/backstage/purchase_cart/content`);
...
async function sendOrder() {
    console.log('發送訂單');
    const response = await fetch(`/backstage/purchase_cart/submit`)
    const data = await response.json();
    if (data.success) {
        alert('訂單已送出');
        updatePage();
