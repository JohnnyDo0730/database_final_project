...
export function init() {
    ...
    // 綁定在容器上（比如整個子頁區塊）
    const container = document.getElementById("purchase_record-page");
    
    if (!container) {
        console.error('找不到 purchase_record-page 元素');
        return;
    }
    ...
    container.addEventListener("click", function (event) {
      ...
      if (targetBtn) {
        console.log("你點擊了採購記錄頁面 的按鈕:", targetBtn.dataset.name);
        // 呼叫該模組內的專屬函數
